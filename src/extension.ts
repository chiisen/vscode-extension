...
export function activate(context: vscode.ExtensionContext): void {
  ...
  console.log(`👍恭喜, 你的 VSCode Extension: "${vscodeExtensionName}" 現在激活了❗`); // 寫到【偵錯主控台】

  // 命令已在 package.json 文件中定義
  // 現在使用 registerCommand 提供命令的實現
  // commandId 參數必須與 package.json 中的命令字段匹配
  const disposable1 = vscode.commands.registerCommand('extension.help', () => {
  ...
  const disposable2 = vscode.commands.registerCommand('extension.createComponent', async () => {
  ...
  const disposable3 = vscode.commands.registerCommand('extension.createNote', createNote);
